...
- **Validation gate**: `bun run check && bun test && bun run build` — all three must pass before any commit. Tests baseline is **314 tests green**; never let the suite shrink.
...
2. `bun test` — **98 or more** tests green (never let the suite shrink)
